...
command -v az >/dev/null 2>&1 || {
	echo >&2 "错误: 'az' (Azure CLI) 未安装或不在 PATH 中。"
	exit 1
}
command -v jq >/dev/null 2>&1 || {
	echo >&2 "错误: 'jq' 未安装或不在 PATH 中。"
	exit 1
}
...
# 查找所有 azure:// 占位符（支持 {{azure://...}} 包裹）
placeholders_wrapped=$(echo "$yaml_content" | grep -o '{{azure://[a-zA-Z0-9\./_-]\+}}' | sort | uniq || true)
placeholders_raw=$(echo "$yaml_content" | grep -o 'azure://[a-zA-Z0-9\./_-]\+' | sort | uniq || true)

if [ -z "$placeholders_wrapped" ] && [ -z "$placeholders_raw" ]; then
...
placeholders=$(printf "%s\n%s\n" "$placeholders_raw" "$placeholders_wrapped" | sed -e 's/^{{//' -e 's/}}$//' | sed '/^$/d' | sort | uniq)
...
for placeholder in $placeholders_wrapped; do
	[ -z "$placeholder" ] && continue
	placeholder_inner="${placeholder#\{\{}"
	placeholder_inner="${placeholder_inner%\}\}}"

	path_part="${placeholder_inner#azure://}"
	IFS='/' read -r vault_name secret_name json_key <<<"$path_part"
...
for placeholder in $placeholders_raw; do
	[ -z "$placeholder" ] && continue
	path_part="${placeholder#azure://}"
	IFS='/' read -r vault_name secret_name json_key <<<"$path_part"
